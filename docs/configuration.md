...
| `CONTROL_PLANE_PUBLIC_URL` | **Yes** (any non-localhost deploy) | `http://localhost:8000` | Externally-reachable base URL of the control-plane API, e.g. `https://cp.${DOMAIN_BASE}`. uvicorn runs without `--proxy-headers`, so `Request.base_url` would report the internal container address; this value is what `POST /shares/{id}/file-token` returns as `base_url`. If left at the default on a public deployment, file attachments are not transferred while WebSocket sync still looks healthy. Startup logs a one-time `WARNING` naming this variable when it is the default and `RELAY_PUBLIC_URL` is not a localhost address. Never fails startup: a localhost value is legitimate for single-machine installs. |
