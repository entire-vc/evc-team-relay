...
#   SENTINEL_FILE        - path to write last-success timestamp (default: /backups/.last-offsite-mirror)
...
SENTINEL_FILE="${SENTINEL_FILE:-/backups/.last-offsite-mirror}"
...
if mc mirror --overwrite --remove "${SRC}" "${DST}"; then
    ...
    # Write sentinel so backup-offsite-alert.sh can check mirror run time,
    # not object modification time (incremental mirror has no new objects when
    # docs are unchanged — causes false-positive "stale" alerts).
    date +%s > "${SENTINEL_FILE}" && log_info "Sentinel written: ${SENTINEL_FILE}"
else
