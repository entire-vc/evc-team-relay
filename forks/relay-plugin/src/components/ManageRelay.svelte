<script lang="ts">
	...
	async function onChoose(
		...
		// Automatically scan and sync all files in the folder after share creation
		// This ensures attachments and other files are immediately synced
		try {
			// Wait for the folder to be ready and synced before triggering file sync
			await folder.whenReady();
			await folder.whenSynced();

			// Trigger sync for all files in the shared folder
			// This includes markdown documents, canvases, and attachments
			plugin.backgroundSync.enqueueSharedFolderSync(folder);

			new Notice(`Syncing files in "${normalizedPath}"...`);
		} catch (error) {
			console.error("Error triggering automatic sync:", error);
			new Notice(`Failed to start automatic sync: ${error.message}`);
		}
